...
from PySide6.QtWidgets import (
    ...
    QTextEdit, QSplitter, QFrame, QScrollArea
)
...
class StrategyBuilderWidget(QWidget):
    ...
    def _on_new_strategy(self):
        ...
        if ok and name:
            description, ok = QInputDialog.getText(self, "New Strategy", "Strategy description (optional):")
            if ok:
    ...
    def _on_add_signal(self):
        ...
        if not current_item:
            ...
            QMessageBox.information(self, "No Signal Selected", "Please select a signal from the list first.")
            return
        ...
        if not self.strategy_model.has_strategy():
            strategy_name = self.strategy_name_edit.text().strip() or f"Strategy {signal_type.value.title()}"
            strategy_desc = self.strategy_desc_edit.toPlainText().strip() or f"Strategy using {signal_type.value.title()}"
            self.strategy_model.create_strategy(strategy_name, strategy_desc)
        
        # Show role selection dialog with modern styling
        from PySide6.QtWidgets import QDialog, QDialogButtonBox, QComboBox, QVBoxLayout, QLabel
    ...
    def _on_signal_widget_updated(self, signal_id: str):
        ...
        if signal_widget:
            for param_name, param_widget in signal_widget.parameter_widgets.items():
                ...
                self.strategy_model.update_signal_parameter(signal_id, param_name, value)
