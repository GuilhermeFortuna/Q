...
class EquityCurveWidget(QWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.figure = Figure(figsize=(12, 6), facecolor=theme.BACKGROUND_MAIN)
    ...
    def _apply_styling(self):
        """Apply chart styling."""
        self.ax.set_facecolor(theme.BACKGROUND_MAIN)
...
class ResultsPanelWidget(QWidget):
    ...
    def _create_summary_tab(self):
        ...
        # Create metric cards
        self.metrics_cards = {
            'total_return': MetricsCardWidget(
                "Total Return", "$0.00", "Total profit/loss"
            ),
            'sharpe_ratio': MetricsCardWidget(
                "Sharpe Ratio", "0.00", "Risk-adjusted return"
            ),
            'max_drawdown': MetricsCardWidget(
                "Max Drawdown", "0.00%", "Largest peak-to-trough decline"
            ),
            'win_rate': MetricsCardWidget(
                "Win Rate", "0.00%", "Percentage of profitable trades"
            ),
            'profit_factor': MetricsCardWidget(
                "Profit Factor", "0.00", "Gross profit / Gross loss"
            ),
            'total_trades': MetricsCardWidget(
                "Total Trades", "0", "Number of trades executed"
            ),
            'avg_trade': MetricsCardWidget(
                "Avg Trade", "$0.00", "Average profit per trade"
            ),
            'avg_duration': MetricsCardWidget(
                "Avg Duration", "0h", "Average trade duration"
            ),
        }
    ...
    def _update_summary_metrics(self, results_data: Dict[str, Any]):
        ...
        self.metrics_cards['max_drawdown'].set_value(
            f"{max_dd:.2f}%", theme.ACCENT_DANGER
        )
