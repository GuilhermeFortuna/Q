...
from PySide6.QtWidgets import (
    ...
    QTimeEdit, QCheckBox, QTabWidget, QScrollArea
)
...
class BacktestConfigWidget(QWidget):
    ...
    def _connect_signals(self):
        ...
        widgets = [
            self.point_value_spin, self.cost_per_trade_spin, self.initial_capital_spin,
            self.commission_spin, self.margin_requirement_spin, self.max_trades_per_day_spin,
            self.permit_swingtrade_check, self.max_position_size_spin, self.stop_loss_pips_spin,
            self.take_profit_pips_spin, self.entry_time_edit, self.exit_time_edit,
            self.slippage_spin, self.bypass_first_exit_check
        ]
    ...
    def _load_config(self):
        ...
        if config.entry_time_limit:
            self.entry_time_edit.setTime(QTime.fromString(str(config.entry_time_limit), "hh:mm:ss"))
        if config.exit_time_limit:
            self.exit_time_edit.setTime(QTime.fromString(str(config.exit_time_limit), "hh:mm:ss"))
    ...
    def _on_config_changed(self):
        ...
        config.initial_capital = self.initial_capital_spin.value() if self.initial_capital_spin.value() > 0 else None
        config.commission = self.commission_spin.value()
        config.margin_requirement = self.margin_requirement_spin.value()
        
        # Risk management
        config.max_trade_day = self.max_trades_per_day_spin.value() if self.max_trades_per_day_spin.value() > 0 else None
        config.permit_swingtrade = self.permit_swingtrade_check.isChecked()
        config.max_position_size = self.max_position_size_spin.value() if self.max_position_size_spin.value() > 0 else None
        config.stop_loss_pips = self.stop_loss_pips_spin.value() if self.stop_loss_pips_spin.value() > 0 else None
        config.take_profit_pips = self.take_profit_pips_spin.value() if self.take_profit_pips_spin.value() > 0 else None
        
        # Time limits
        config.entry_time_limit = self.entry_time_edit.time().toPython() if self.entry_time_edit.time().isValid() else None
        config.exit_time_limit = self.exit_time_edit.time().toPython() if self.exit_time_edit.time().isValid() else None
