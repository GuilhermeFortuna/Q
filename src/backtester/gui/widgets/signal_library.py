...
class SignalLibraryWidget(QWidget):
    """
    Widget for browsing and learning about available trading signals.
    ...
    def _create_parameter_widget(self, param_name: str, param_config) -> QWidget:
        ...
        widget.setStyleSheet("QFrame { border: 1px solid #ccc; border-radius: 3px; padding: 5px; }")
        ...
        if param_config.min_value is not None and param_config.max_value is not None:
            value_info.append(f"Range: {param_config.min_value} - {param_config.max_value}")
        elif param_config.options:
            value_info.append(f"Options: {', '.join(map(str, param_config.options))}")
        
        if param_config.required:
            value_info.append("Required")
        else:
            value_info.append("Optional")
        
        if value_info:
            value_label = QLabel(" | ".join(value_info))
            value_label.setStyleSheet("color: blue; font-size: 9px;")
            layout.addWidget(value_label)
        
        return widget
    
    def _update_usage_examples(self, signal_type: SignalType):
        """Update the usage examples display."""
        examples = self._get_usage_examples(signal_type)
        self.usage_examples.setPlainText(examples)
