...
from PySide6.QtWidgets import (
    ...
    QComboBox, QLineEdit, QDateEdit, QSpinBox, QTableWidget, QTableWidgetItem,
    QTabWidget, QTextEdit, QProgressBar, QFileDialog, QMessageBox,
    QFormLayout, QCheckBox, QSplitter, QHeaderView, QAbstractItemView,
    QScrollArea
)
...
class DataSourceWidget(QWidget):
    ...
    def _setup_ui(self):
        ...
        form_layout.addRow("File Path:", self.file_path_layout)
    ...
    def _update_ui_visibility(self):
        ...
        self.file_path_layout.parent().setVisible(is_file_source)
...
class DataPreviewWidget(QWidget):
    ...
    def _get_data_statistics(self, df, source_id: str) -> str:
        ...
        stats = [
            ...
            f"Date Range: {df.index.min()} to {df.index.max()}" if hasattr(df.index, 'min') else "No date range",
...
class DataConfigWidget(QWidget):
    ...
    def _setup_connections(self):
        ...
        self.backtest_model.data_loading_progress.connect(self._on_data_loading_progress)
            ...
            for source_id, data in loaded_data.items():
                ...
                if hasattr(data, 'data') and data.data is not None:
                    ...
                    if hasattr(df.index, 'min'):
                        stats_text.append(f"Date Range: {df.index.min()} to {df.index.max()}")
