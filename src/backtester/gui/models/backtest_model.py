...
class BacktestModel(QObject):
    ...
    def _validate_data_source(self, source_id: str, config: DataSourceConfig):
        ...
        if not config.symbol:
            self._validation_errors.append(f"Data source {source_id}: Symbol is required")
        
        if not config.timeframe:
            self._validation_errors.append(f"Data source {source_id}: Timeframe is required")
        
        if config.source_type in ["csv", "parquet"] and not config.file_path:
            self._validation_errors.append(f"Data source {source_id}: File path is required for file-based sources")
        
        if config.source_type == "database" and not config.connection_string:
            self._validation_errors.append(f"Data source {source_id}: Connection string is required for database sources")
    ...
    def _validate_backtest_config(self):
        ...
        if self._backtest_config.initial_capital is not None and self._backtest_config.initial_capital <= 0:
            self._validation_errors.append("Initial capital must be greater than 0")
        
        if self._backtest_config.max_trade_day is not None and self._backtest_config.max_trade_day <= 0:
    ...
    def get_data(self) -> Dict[str, Any]:
        ...
        # In a real implementation, this would return the actual loaded data
        return {}
    
    def has_data(self) -> bool:
        """Check if any data has been loaded."""
        # This is a placeholder implementation
        # In a real implementation, this would check if data is actually loaded
        return len(self._data_sources) > 0
    
    def get_ohlc_data(self) -> Optional[Any]:
        """Get OHLC data for visualization."""
        # This is a placeholder implementation
        # In a real implementation, this would return actual OHLC data
        return None
        ...
        try:
            ...
            config_data = {
                ...
                "backtest_config": {
                    ...
                    "max_trade_day": self._backtest_config.max_trade_day,
                    "permit_swingtrade": self._backtest_config.permit_swingtrade,
                    "entry_time_limit": self._backtest_config.entry_time_limit.isoformat() if self._backtest_config.entry_time_limit else None,
                    "exit_time_limit": self._backtest_config.exit_time_limit.isoformat() if self._backtest_config.exit_time_limit else None,
        ...
        try:
            ...
            for source_id, source_data in config_data.get("data_sources", {}).items():
                config = DataSourceConfig(
                    ...
                    date_from=datetime.fromisoformat(source_data["date_from"]).date() if source_data.get("date_from") else None,
                    date_to=datetime.fromisoformat(source_data["date_to"]).date() if source_data.get("date_to") else None,
            ...
            self._backtest_config = BacktestConfig(
                ...
                entry_time_limit=time.fromisoformat(backtest_data["entry_time_limit"]) if backtest_data.get("entry_time_limit") else None,
                exit_time_limit=time.fromisoformat(backtest_data["exit_time_limit"]) if backtest_data.get("exit_time_limit") else None,
                slippage=backtest_data.get("slippage", 0.0),
                bypass_first_exit_check=backtest_data.get("bypass_first_exit_check", False),
